...
interface TopPagesProps extends StandardApi {
  data: {
    page: string;
    count: number;
  }[];
...
function TopPagesComponent() {
  ...
  const fetchDataFilteredByDate = async () => {
    ...
    setIsLoading(true);
    await axios
      .get<TopPagesProps>(`${env.NEXT_PUBLIC_API_URL}/top-pages`, {
            ...
            {data?.map((item, index) => (
              ...
              >
                <div className="flex w-full items-center justify-between rounded-lg bg-gray-100 px-3 py-3">
                  <span className="break-all text-xs">{item.page}</span>
...
export default TopPagesComponent;
